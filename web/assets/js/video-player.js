...
$(document).ready(function () {
    ...
    //window.exceptSymbols = ['.', ',', ';', ':', '/', '|', '\\', '?', '!', '~', '`']; // this symbols don't need translation
    ...
    $('.vjs-caption-overlay-text').hover(function (e) {
        bumbuPlayer.pause();
    }, function () {
    ...
    // subtitle translation on hover
    $(document).on('mouseenter', '.s-node', function () {
        var hoveredSubtitle = $(this);
        hoveredSubtitle.addClass('active');
        var text = hoveredSubtitle.text();

        // lets center translate popup with subtitle
        var subtitleWidth = hoveredSubtitle.width(); // subtitle width

        // 230 is popup's window width with translate
        var left = (230 - subtitleWidth) / 2; // this is value (in px) we should move .trans-wr to left

        // hiding all trans-wr
        $('.trans-wr').hide();

        if (hoveredSubtitle.siblings('.trans-wr').length) {
            // already translated
            hoveredSubtitle.siblings('.trans-wr').show().css({'left' : -1 * parseInt(left) + 'px'});
        } else {
            // translation required
            var node = hoveredSubtitle
                .closest('.s-node-wr')
                .append('<span class="trans-wr" style="left: ' + (-1 * parseInt(left) + 'px') + '">Loading...</span>');

            $.post(translateApiUrl, {q: text, _csrf: _csrf}, function (res) {
                var orig = res.data.sentences[0].orig;
                var trans = (res.data.sentences[0].trans) ? res.data.sentences[0].trans : orig;

                node
                    .find('.trans-wr')
                    .text('')
                    .append('<span class="s-orig">' + orig + '</span>')
                    .append('<span class="s-trans">' + trans + '</span>')
                    .append('<button class="btn btn-sm btn-outline" type="button" title="Add to my dictionary">+</button>');
            });
        }
    });

    $(document).on('mouseleave', '.s-node-wr', function () {
        // hiding all trans-wr
        $('.trans-wr').hide();
        $(this).find('.s-node').removeClass('active');
    });
