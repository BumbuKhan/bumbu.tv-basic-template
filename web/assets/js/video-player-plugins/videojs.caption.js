    ...
    function registerCaptionPlugin(options) {
        ...
        function updatePopOnCaptionText() {
            ...
            while (true) {
                ...
                // here we need to wrap every word in span tag, so that we'll be able to hang hover event and
                var words = captionText.split(" ");

                var newString = ''; // clear

                // we have to determine symbols which do not need translation; such as , ; . ' " ! ? and so forth
                ...
                $.each(words, function (i, v) {
                    ...
                    newString += '<span class="s-node-wr"><span class="s-node">' + v + '</span></span>' + divider;
                });

                captionText = newString;
